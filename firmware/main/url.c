...
//#define BADGE_HOST "192.168.0.4.nip.io"
...
mbedtls_x509_crt * getclicert(){
  return clicert;
};

mbedtls_pk_context * getclicertpk(){
  return pk_ctx_clicert;
};
...
int initmbtls()
{
  ...
  if(!mbtlsinited){
    ...
    if( (ret=mbedtls_x509_crt_parse(
				    certchain,server_root_cert_pem_start,
...
void solvebadgehost()
{
  ...
  if(hostfound == 2){ // can't resolve, try again once
    hostfound = 0;
    dns_gethostbyname(badgehost, &badgesrvip, dns_found_cb, NULL );
    while(hostfound == 0);
  }
...
int gen_rsakp_and_csr(mbedtls_pk_context * pubkey,unsigned char* buff,uint16_t buffSize)
{
  ...
  sprintf((char*)buff,"O=%s,OU=%s,L=%s,CN=%s","BruCON","Badges","BruCON_StrikesAgain<AH:AH:2021>",(char*)macstr);
  ...
  if(mbedtls_rsa_gen_key(rsa_ctx_dcert, mbedtls_ctr_drbg_random, ctr_drbg_dcert,
			 //1024
			 2048
			 , 65537) != 0){
