...
#include "expat.h"
#include "wifi.h"
#if defined(CONFIG_HARDWARE_2020) || defined(CONFIG_HARDWARE_2019)
#include "touch.h"
#include "math.h"
#include "leds.h"
#include <soc/soc.h>
...
volatile char notaskbar = 0;

void display_skeleton();
void display_taskbar();

void setnotaskbar(int i){
  notaskbar=i;
}
...
void VivaLaToothpaste(void * arg)
{
  notaskbar=1;
  // for you richard...
  fakeplaceholder.parent = currMenuItem;
...
uint32_t VivaLaToothpasteDirect(void * arg)
{
  // for you richard...
  fakeplaceholder.parent = currMenuItem;
  currMenuItem = &fakeplaceholder;
  /*uint32_t score = */
  uint32_t r=VivaLaVodkaL((void*)1);
  skeleton_inited = 0;
  return r;
}
...
void EraseKeys(void* arg){
  if( getBruCONConfigFlag("haveKeys") ){
    printf("Erasing keys!\n");
    BruCONErase_key("haveKeys");
    BruCONErase_key("haveClientCert");
    BruCONErase_key("ClientCert");
    BruCONErase_key("POWER_HISTORY");
    BruCONErase_key("UNIQUE_ID");
    BruCONErase_key("EVERSION");
    BruCONErase_key("haveAlcCal");
    printf("Erased keys!\n");
  }
  
}

void RebootF(void* arg){
  printf("REBOOT!\n");
  esp_restart();
}

void testMQTT(void* arg)
{
  printf("TEST MQTT !\n");
  MQTT_Start();
}
...
void testF1(void* arg)
{
  printf("TEST 1!\n");
  nextwheelpow(25);
}
void testF2(void* arg)
{
  printf("TEST 2!\n");
  //taskColorWheel();
  taskColorAdvertise();
...
#pragma message "CONFIG2019"
    ...
    if(menu_dirty){
        display_skeleton(); display_taskbar();
        menu_dirty=0;
    }
